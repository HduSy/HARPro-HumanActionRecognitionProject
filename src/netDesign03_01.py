...
from src.public import txtDir, actions, model_filename, spatial_attention, temporal_attention, frameNum, inputA_dim, \
    inputB_dim, \
    anglization
...
def train(x1_train, x2_train, y1_train, x1_vali, x2_vali, y1_vali):
    ...
    # 定义两个分支
    inputA = Input(shape=(frameNum, inputA_dim))
    inputB = Input(shape=(frameNum, inputB_dim))
    # 空间注意力模块-空间关键点选择门
    # return_sequences:true返回所有中间隐藏值false返回最后一个隐藏值
    # shape=(samples, frameNum, input_dim)
    if spatial_attention:
        print('空间注意力机制')
        x1 = GRU(n_hidden, batch_input_shape=(None, frameNum, inputA_dim), return_sequences=True,
                 unroll=True)(inputA)
